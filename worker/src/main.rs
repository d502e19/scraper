...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let args = App::new("DatScraper Worker")
        ...
        .arg(
            ...
            Arg::with_name("redis-sentinel-set")
                .long("redis-sentinel-set")
                .env("SCRAPER_REDIS_SENTINEL_SET")
                .default_value("master")
                .value_name("SET")
                .help("Specify the Redis Sentinel set")
        ).arg(
    ...
    if let Ok(_handle) = log4rs::init_config(get_log4rs_config(
        ...
        let manager = RMQRedisManager::new(
            ...
            if args.value_of("sentinel").unwrap().parse::<bool>().expect("The sentinel argument was not a boolean") == true {
                Some(args.value_of("redis-sentinel-set").unwrap().to_string())
            } else {
                None
            }
        )
